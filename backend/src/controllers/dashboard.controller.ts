...
export const getSummary = async (req: Request, res: Response) => {
  ...
  const [todayTasks, overdueTasks, next7DaysTasks, activeProjects, completedTasks, totalTasks] =
    await Promise.all([
  ...
  res.json({ todayTasks, overdueTasks, next7DaysTasks, activeProjects, completedTasks, totalTasks, completionPercentage });
};
